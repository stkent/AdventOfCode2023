...
infix fun <T> Iterable<T>.intersects(other: Iterable<T>): Boolean = intersect(other.toSet()).isNotEmpty()
...
fun <T> Iterable<T>.partitionBy(connected: (T, T) -> Boolean): Set<List<T>> {
    ...
    forEach { newElement ->
        ...
        result.apply {
            removeAll(partsToConnect.toSet())
